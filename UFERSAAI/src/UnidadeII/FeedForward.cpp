...
void erroRede(FeedForward *ff) {
	...
	ff->erro = pow(ff->mlp.perceptron[0].neuronio.saida.propagacao, 2.0);
	for (int neuronio = 1; neuronio < numNeuronios; ++neuronio) {
		ff->erro += pow(ff->mlp.perceptron[neuronio].neuronio.saida.propagacao,
				2.0);
	}
	ff->erro = ff->erro / 2.0;
	// cout << ff->erro << endl;
...
//	srand(t*time(NULL));
	...
	do {
		 ...
		 *	  No qual dk é o valor desejado na saida e fks’(netks) é
		 *	  a derivada da função de transferência do neurônio k da camada de saída.
		 */
		...
		limite++;
	} while ((float)abs(result - saidaDesejada) > 100);
